    ...
    <section ref={heroSectionRef} className={styles.heroSection}>
      <div className={styles.heroContent}>
        <h1 className={`${styles.heroTitle} ${loaded ? styles.fadeIn : ''}`}>
          Encontre os <span className={styles.barbantesColor}>Barbantes</span> <br />
          para seus Projetos de Crochê
        </h1>
        <p className={`${styles.heroSubtitle} ${loaded ? styles.fadeInDelay : ''}`}>
          Qualidade e variedade de fios para você criar peças únicas e encantadoras.
        </p>
        <Link
          to="barbantes"
          className={`${styles.heroButton} ${loaded ? styles.fadeInDelayMore : ''}`}
        >
          Ver Barbantes
        </Link>
      </div>
